      ...
      try {
        ...
        setPairs((currentPairs) =>
          currentPairs.map((pair) => {
            const binancePair = data.find((p) => p.symbol === pair.base + 'USDT')
            if (binancePair) {
              return {
                ...pair,
          ...
          {pairs.map((pair) => (
            ...
            >
              ...
              <div className="text-right">
                <div className="text-[#6b7280]">{pair.price.toFixed(2)}</div>
                <div
                  className={`text-sm ${pair.change >= 0 ? 'text-green-500' : 'text-[#ef4444]'}`}
                >
                  {pair.change >= 0 ? '+' : ''}
